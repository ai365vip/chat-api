...
const vutils_1 = require("@visactor/vutils"), calculateNodeValue = (subTree, output, depth = 0, flattenIndex = -1, parent, getNodeKey) => {
    ...
    return subTree.forEach(((datum, index) => {
        ...
        if (null === (_b = datum.children) || void 0 === _b ? void 0 : _b.length) {
            ...
            const res = (0, exports.calculateNodeValue)(datum.children, node.children, depth + 1, prevFlattenIndex, node, getNodeKey);
            node.value = (0, vutils_1.isNil)(datum.value) ? res.sum : Math.max(res.sum, node.value), 
            prevFlattenIndex = res.flattenIndex, maxDepth = Math.max(res.maxDepth, maxDepth);
        } else node.isLeaf = !0, node.value = (0, vutils_1.isNil)(datum.value) ? 0 : datum.value;
