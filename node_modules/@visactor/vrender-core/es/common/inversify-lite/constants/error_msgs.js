...
export const UNDEFINED_INJECT_ANNOTATION = name => `@inject called with undefined this could mean that the class ${name} has a circular dependency problem. You can use a LazyServiceIdentifer to  overcome this limitation.`;
...
export const LAZY_IN_SYNC = key => `You are attempting to construct '${key}' in a synchronous way\n but it has asynchronous dependencies.`;
...
export const ARGUMENTS_LENGTH_MISMATCH = (...values) => `The number of constructor arguments in the derived class ${values[0]} must be >= than the number of constructor arguments of its base class.`;
...
export const POST_CONSTRUCT_ERROR = (clazz, errorMessage) => `@postConstruct error in class ${clazz}: ${errorMessage}`;

export const PRE_DESTROY_ERROR = (clazz, errorMessage) => `@preDestroy error in class ${clazz}: ${errorMessage}`;

export const ON_DEACTIVATION_ERROR = (clazz, errorMessage) => `onDeactivation() error in class ${clazz}: ${errorMessage}`;

export const CIRCULAR_DEPENDENCY_IN_FACTORY = (factoryType, serviceIdentifier) => `It looks like there is a circular dependency in one of the '${factoryType}' bindings. Please investigate bindings with service identifier '${serviceIdentifier}'.`;
