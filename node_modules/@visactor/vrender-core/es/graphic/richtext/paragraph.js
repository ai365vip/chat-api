...
export default class Paragraph {
    constructor(text, newLine, character) {
        ...
        const {ascent: ascent, height: height, descent: descent, width: width} = measureTextCanvas(text, character);
    ...
    updateWidth() {
        const {width: width} = measureTextCanvas(this.text, this.character);
    ...
    draw(ctx, baseline, deltaLeft, isLineFirst, textAlign) {
        ...
        left = 0, baseline = 0), this.character.stroke && (applyStrokeStyle(ctx, this.character), 
        ctx.strokeText(text, left, baseline)), applyFillStyle(ctx, this.character), this.character.fill && ctx.fillText(text, left, baseline), 
