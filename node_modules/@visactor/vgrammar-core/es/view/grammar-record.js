...
export class RecordedTreeGrammars extends RecordedGrammars {
    ...
    unrecord(grammar) {
        if (super.unrecord(grammar), "mark" === grammar.grammarType) {
            ...
            this._markNodes.forEach((node => {
                const targetMark = node.mark;
                targetMark.markType === GrammarMarkType.group && targetMark.includesChild(mark, !1) ? (node.children = node.children.filter((n => n !== currentNode)), 
                currentNode.parent = null) : mark.markType === GrammarMarkType.group && mark.includesChild(targetMark, !1) && (currentNode.children = currentNode.children.filter((n => n !== node)), 
                node.parent = null);
