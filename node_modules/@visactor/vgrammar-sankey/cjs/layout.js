...
class SankeyLayout {
    ...
    layout(data, config) {
        ...
        return this.computeLinkBreadths(nodes), (0, vgrammar_util_1.isVertical)(this.options.direction) && (nodes.forEach((node => {
    ...
    computeSourceTargetNodeLinks(data) {
        ...
        return data.links.forEach(((link, index) => {
            if ((0, vutils_1.isNil)(link.source) || (0, vutils_1.isNil)(link.target)) return;
    ...
    computeNodeValues(nodes) {
        for (let i = 0, len = nodes.length; i < len; i++) {
            const node = nodes[i];
            node.value = Math.max((0, vutils_1.isNil)(node.value) ? 0 : node.value, node.sourceLinks.reduce(((sum, link) => {
                var _a;
            ...
            for (let i = 0; i < this._maxDepth; i++) {
                ...
                for (let j = 0, colLen = column.length; j < colLen; j++) {
                    ...
                    for (let k = 0, linkLen = sourceLinks.length; k < linkLen; k++) {
                        const link = sourceLinks[k], curLinkWidth = (0, vutils_1.isFunction)(linkWidthOption) ? linkWidthOption(link) : linkWidth;
