...
const bubble_1 = require("./bubble"), vutils_1 = require("@visactor/vutils"), event_1 = require("../constant/event"), componentTypeMap = {
    cartesianAxis: "axis",
...
class EventDispatcher {
    ...
    dispatch(eType, params, level) {
        ...
        if (level) {
            ...
            const handlers = bubble.getHandlers(event_1.Event_Bubble_Level.mark);
            if (stopBubble = this._invoke(handlers, eType, params), !stopBubble) {
                const handlers = bubble.getHandlers(event_1.Event_Bubble_Level.model);
                stopBubble = this._invoke(handlers, eType, params);
            }
            if (!stopBubble) {
                const handlers = bubble.getHandlers(event_1.Event_Bubble_Level.chart);
                stopBubble = this._invoke(handlers, eType, params);
            }
            if (!stopBubble) {
                const handlers = bubble.getHandlers(event_1.Event_Bubble_Level.vchart);
                stopBubble = this._invoke(handlers, eType, params);
            }
        }
        return this;
    ...
    release() {
        for (const entry of this._viewListeners.entries()) this._compiler.removeEventListener(event_1.Event_Source_Type.chart, entry[0], entry[1]);
        this._viewListeners.clear();
        for (const entry of this._windowListeners.entries()) this._compiler.removeEventListener(event_1.Event_Source_Type.window, entry[0], entry[1]);
        this._windowListeners.clear();
        for (const entry of this._canvasListeners.entries()) this._compiler.removeEventListener(event_1.Event_Source_Type.canvas, entry[0], entry[1]);
    ...
    _isValidEvent(eType) {
        return event_1.BASE_EVENTS.includes(eType) || Object.values(event_1.VGRAMMAR_HOOK_EVENT).includes(eType) || Object.values(event_1.ChartEvent).includes(eType);
    }
}
