"use strict";

Object.defineProperty(exports, "__esModule", {
    value: !0
}), exports.DimensionTrigger = void 0;

const vutils_1 = require("@visactor/vutils"), interface_1 = require("../event/events/dimension/interface"), mark_set_1 = require("../mark/mark-set"), interface_2 = require("../compile/mark/interface");

class DimensionTrigger {
    get hover() {
        return this._hover;
    }
    get select() {
        return this._select;
    }
    constructor(option) {
        this._marks = new mark_set_1.MarkSet, this._markReverse = new mark_set_1.MarkSet, 
        this._lastDimensionEvent = null, this.onHover = params => {
            const elements = this.getEventElement(params);
            switch (params.action) {
              case "enter":
                this.interaction.getEventElement(interface_2.STATE_VALUE_ENUM.STATE_DIMENSION_HOVER).forEach((e => {
                    this.interaction.addEventElement(interface_2.STATE_VALUE_ENUM.STATE_DIMENSION_HOVER_REVERSE, e);
                })), this.interaction.clearEventElement(interface_2.STATE_VALUE_ENUM.STATE_DIMENSION_HOVER, !1), 
                elements.forEach((({el: el, mark: mark}) => {
                    this.interaction.addEventElement(interface_2.STATE_VALUE_ENUM.STATE_DIMENSION_HOVER, el);
                })), this.interaction.reverseEventElement(interface_2.STATE_VALUE_ENUM.STATE_DIMENSION_HOVER);
                break;

              case "leave":
                this.interaction.clearEventElement(interface_2.STATE_VALUE_ENUM.STATE_DIMENSION_HOVER, !0), 
                params = null;
            }
            this._lastDimensionEvent = params;
        }, this._option = option, this.event = this._option.model.getOption().getChart().getEvent(), 
        this.interaction = option.interaction, this.initConfig(option.mode);
    }
    setStateKeys(fields) {}
    registerMark(mark) {
        (0, vutils_1.isEmpty)(mark.stateStyle[interface_2.STATE_VALUE_ENUM.STATE_DIMENSION_HOVER]) || this._marks.addMark(mark), 
        (0, vutils_1.isEmpty)(mark.stateStyle[interface_2.STATE_VALUE_ENUM.STATE_DIMENSION_HOVER_REVERSE]) || this._markReverse.addMark(mark);
    }
    init() {
        this.initEvent();
    }
    release() {
        this.releaseEvent();
    }
    initEvent() {
        this.event.on(interface_1.DimensionEventEnum.dimensionHover, this.onHover);
    }
    releaseEvent() {
        this.event.release();
    }
    initConfig(mode) {}
    getEventElement(params, reverse = !1) {
        const items = [];
        return params.dimensionInfo.forEach((df => {
            df.data.forEach((dd => {
                (reverse ? this._markReverse : this._marks).getMarks().filter((m => m.model === dd.series && m.getVisible())).forEach((m => {
                    const elements = m.getProduct().elements.filter((e => {
                        const datum = e.getDatum();
                        let c;
                        return c = (0, vutils_1.isArray)(datum) ? datum.every(((oneData, i) => oneData === dd.datum[i])) : dd.datum.some((dd_d => dd_d === datum)), 
                        reverse ? !c : c;
                    })).map((el => ({
                        el: el,
                        mark: m
                    })));
                    items.push(...elements);
                }));
            }));
        })), items;
    }
    dimensionDataEqual(params) {
        if (this._lastDimensionEvent === params) return !0;
        if (!this._lastDimensionEvent || !params) return !1;
        if (this._lastDimensionEvent.dimensionInfo.length !== params.dimensionInfo.length) return !1;
        for (let i = 0; i < this._lastDimensionEvent.dimensionInfo.length; i++) {
            const last = this._lastDimensionEvent.dimensionInfo[i], current = params.dimensionInfo[i];
            if (last.value !== current.value) return !1;
            if (last.axis !== current.axis) return !1;
            if (last.data.length !== current.data.length) return !1;
            for (let j = 0; j < last.data.length; j++) {
                const lastData = last.data[j], currentData = current.data[j];
                if (lastData.series !== currentData.series) return !1;
                if (lastData.datum.length !== currentData.datum.length) return !1;
                for (let k = 0; k < lastData.datum.length; k++) if (lastData.datum[k] !== currentData.datum[k]) return !1;
            }
        }
        return !0;
    }
}

exports.DimensionTrigger = DimensionTrigger;
//# sourceMappingURL=dimension-trigger.js.map