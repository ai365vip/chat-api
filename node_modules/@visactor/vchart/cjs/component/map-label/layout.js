...
function layoutOuter(pairs, features, dataToPosition) {
    ...
    vutils_1.getAABBFromPoints)(_points), centerPosition = dataToPosition([ (x1 + x2) / 2, (y1 + y2) / 2 ]);
...
function nearestPoint(features, origin, bearing, distance = 200) {
    ...
    for (let i = 1; i <= count; i++) {
        const dest = (0, vutils_1.destination)(curOrigin, distance, bearing);
...
function isPointWithinFeatures(features, p) {
    for (let i = 0; i < features.length; i++) {
        if ((0, vutils_1.isPointInPolygon)(p, features[i])) return !0;
    }
    return !1;
}
