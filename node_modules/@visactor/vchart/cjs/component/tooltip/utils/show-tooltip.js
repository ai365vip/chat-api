...
const type_1 = require("../../../series/interface/type"), constant_1 = require("../../../constant"), vutils_1 = require("@visactor/vutils"), vchart_1 = require("../../../core/vchart"), getDataArrayFromFieldArray = (fields, datum) => (0, 
...
function showTooltip(datum, options, tooltipHandler, componentOptions) {
    ...
    const opt = Object.assign({
        regionIndex: 0
    }, options), region = componentOptions.getRegionsInUserIdOrIndex((0, vutils_1.isValid)(opt.regionId) ? [ opt.regionId ] : void 0, (0, 
    vutils_1.isValid)(opt.regionIndex) ? [ opt.regionIndex ] : void 0)[0];
    ...
    seriesList.forEach((series => {
        ...
        vutils_1.isValid)(groupField) && (0, vutils_1.isNil)(groupData) && groupDomain.length > 0;
        if ("cartesian" === series.coordinate) {
            const cartesianSeries = series, invalidDimensionFields = dimensionFields.map(((field, i) => [ field, i ])).filter((([, i]) => (0, 
        ...
        } else if ("polar" === series.coordinate) if (series.type === type_1.SeriesTypeEnum.pie) {
            const pieSeries = series, originDatum = null === (_e = pieSeries.getViewData()) || void 0 === _e ? void 0 : _e.latestData.find(datumContainsArray(dimensionFields, dimensionData));
            if (!hasMeasureData && (measureData = getDataArrayFromFieldArray(measureFields, originDatum), 
            !hasData(measureData))) return;
            ...
            if (isMultiGroups) {
                const originDatum = (null === (_f = polarSeries.getViewData()) || void 0 === _f ? void 0 : _f.latestData.filter(datumContainsArray(dimensionFields, dimensionData))).find((d => d[groupField] === groupData));
                groupDomain.forEach((groupData => {
                    if (measureData = getDataArrayFromFieldArray(measureFields, originDatum), !hasData(measureData)) return;
        ...
        } else if ("geo" === series.coordinate) {
            const gs = series, originDatum = null === (_h = gs.getViewData()) || void 0 === _h ? void 0 : _h.latestData.find(datumContainsArray(dimensionFields, dimensionData));
            if (!hasMeasureData && (measureData = getDataArrayFromFieldArray(measureFields, originDatum), 
            !hasData(measureData))) return;
    ...
    const activeType = null !== (_a = opt.activeType) && void 0 !== _a ? _a : markInfoList.length > 1 ? "dimension" : "mark", regionPos = region.getLayoutStartPoint(), regionRect = region.getLayoutRect(), container = componentOptions.globalInstance.getContainer(), containerPos = Object.assign({
        x: 0,
        y: 0
