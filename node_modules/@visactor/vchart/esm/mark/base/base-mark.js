...
export class BaseMark extends CompilableMark {
    ...
    initStyleWithSpec(spec, key) {
        spec && (isValid(spec.id) && (this._userId = spec.id), isBoolean(spec.interactive) && (this._interactive = spec.interactive), 
        isValid(spec.zIndex) && this.setZIndex(spec.zIndex), isBoolean(spec.visible) && this.setVisible(spec.visible), 
        this._initSpecStyle(spec, this.stateStyle, key));
    }
    convertAngleToRadian(styleConverter) {
        if (isNumber(styleConverter)) return degreeToRadian(styleConverter);
        if (styleConverter.scale) {
            const range = styleConverter.scale.range();
            return styleConverter.scale.range(range.map(degreeToRadian)), styleConverter;
        }
