...
export class Coordinate {
    ...
    start(x, y) {
        return isNil(x) ? this.startPoint : (this.startPoint = this._parsePoint(x, y), this._updateSize(), 
        this);
    }
    end(x, y) {
        return isNil(x) ? this.endPoint : (this.endPoint = this._parsePoint(x, y), this._updateSize(), 
        this);
    }
    _parsePoint(x, y) {
        const point = {
        ...
        return isArray(x) ? (point.x = x[0], point.y = x[1]) : isNumber(x) ? (point.x = x, 
    ...
    _invokeTransforms() {
        this.invertMatrix = null, this.convertMatrix = null, this.invertMatrix = new Matrix, 
