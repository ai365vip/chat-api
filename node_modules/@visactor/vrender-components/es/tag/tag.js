...
export class Tag extends AbstractComponent {
    ...
    render() {
        var _a, _b, _c, _d;
        const {text: text = "", textStyle: textStyle = {}, shape: shape = {}, panel: panel = {}, space: space = 4, minWidth: minWidth, maxWidth: maxWidth, padding: padding = 4, visible: visible, state: state, type: type, textAlwaysCenter: textAlwaysCenter} = this.attribute, parsedPadding = normalizePadding(padding), group = this.createOrUpdateChild("tag-content", {
            x: 0,
            y: 0,
            zIndex: 1
        }, "group");
        ...
        if (tagWidth += symbolPlaceWidth, textX += symbolPlaceWidth, "rich" === type) {
            ...
            tagWidth += textWidth;
            const size = null !== (_d = shape.size) && void 0 !== _d ? _d : 10, maxSize = isNumber(size) ? size : Math.max(size[0], size[1]);
