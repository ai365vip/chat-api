	...
	for {
		...
		if len(tasks) != 0 {
			...
			for _, task := range tasks {
				...
				mjData, _, err := model.GetMJAndChannelData(task.MjId, task.ChannelId)
				if mjData == nil || err != nil {
					log.Printf("hook回调失败: %s", task.MjId)
					...
					// 从mj表中获取字段并赋值给任务
					task.Code = 1
					task.Progress = mjData.Progress
					task.PromptEn = mjData.PromptEn
					task.State = mjData.State
					task.SubmitTime = mjData.SubmitTime
					task.StartTime = mjData.StartTime
					task.FinishTime = mjData.FinishTime
					task.ImageUrl = mjData.ImageUrl
					task.Status = mjData.Status
					task.FailReason = mjData.FailReason

					if task.Progress != "100%" && mjData.FailReason != "" {
						log.Println(task.MjId + " 构建失败，" + task.FailReason)
						task.Progress = "100%"
						err = model.CacheUpdateUserQuota(task.UserId)
						if err != nil {
