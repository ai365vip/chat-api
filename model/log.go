...
func GetUserLogs(userId int, logType int, startTimestamp int64, endTimestamp int64, modelName string, tokenName string, startIdx int, num int) ([]*Log, int64, error) {
	...
	// 获取具体的日志记录
	err = tx.Select("id", "user_id", "created_at", "type", "username", "token_name", "model_name", "quota", "prompt_tokens", "completion_tokens", "token_id", "use_time", "is_stream", "multiplier", "user_quota").
		Order("id desc").
		Limit(num).
		Offset(startIdx).
		Find(&logs).Error
